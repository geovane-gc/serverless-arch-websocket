...
export const handler = async (event: APIGatewayProxyEvent) => {
  try {
    ...
    if (!body || !body?.channel) throw new Error('Channel is required');
    ...
    if (channel == 'watchticker' && !!body)
      channel = `${channel}#${body?.symbol}`;

    if (body?.action == 'subscribe') {
      if (channels.length > 0) {
        channels = await channels.filter(
          (item: any) => item.toLowerCase() != channel,
        );
      }
    ...
    return {
      statusCode: 200,
      body: JSON.stringify('Data successfully saved.'),
    };
  } catch (err: any) {
    return {
      statusCode: 500,
